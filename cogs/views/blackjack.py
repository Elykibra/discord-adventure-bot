...
class CustomBetModal(discord.ui.Modal, title="Custom Bet"):
    ...
    def __init__(self, bet_view: "BlackjackBetView", *, already_public: bool = False):
        super().__init__()
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # fix as Baccarat's BetModal and the same shape already proven
        # correct in Poker's RaiseModal.
        if view.busy:
...
class PlayAgainButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        view: BlackjackResultView = self.view
        db_cog = interaction.client.get_cog('Database')
        wallet = await db_cog.get_or_create_wallet(interaction.user.id)
